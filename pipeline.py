...
LUA_SHA1 = get_hash(os.path.join(CWD, 'telenor.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--timeout", "30",
            "--tries", "inf",
            "--domains", "online.no,148.122.161.133,frisurf.no",
            "--span-hosts",
            "--waitretry", "30",
            "--warc-file", ItemInterpolation("%(item_dir)s/%(warc_file_base)s"),
            "--warc-header", "operator: Archive Team",
            "--warc-header", "telenor-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("telenor-user: %(item_name)s"),
        ]
        ...
        if item_type == 'telenor':
            ...
            wget_args.append('http://{0}.hos.frisurf.no/'.format(item_value))
...
project = Project(
    title="telenor",
    project_html="""
        <img class="project-logo" alt="Project logo" src="http://archiveteam.org/images/9/96/Telenor_logo.png" height="50px" title=""/>
        <h2>home.online.no <span class="links"><a href="http://home.online.no/">Website</a> &middot; <a href="http://tracker.archiveteam.org/telenor/">Leaderboard</a></span></h2>
        <p>Telenor is killing their users personal homepages.</p>
    """,
    utc_deadline=datetime.datetime(2015, 11, 1, 23, 59, 0)
)
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="telenor"),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
